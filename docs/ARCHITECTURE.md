...
6. The ingest queue only enqueues *newly captured* observations. The long tail (rows ingested before the classifier was wired up, or whose first attempt errored) is reached by a separate Vercel cron at `/api/cron/classify-backfill` (every 6 h at 03/09/15/21 UTC, staggered 3 h from the scrape ticks). It walks `mv_observation_current` for rows where `is_canonical = true AND llm_classified_at IS NULL`, sorts by `impact_score DESC, published_at DESC`, and routes the top N through the same `processObservationClassificationQueue()` helper with `reclassifyExisting: false`. It is **impact-ungated by default** (`minImpactScore=0`): the prior `impact_score >= 6` gate permanently skipped the below-threshold tail, so coverage regressed as the corpus grew (16% → 10.9%, 2026-05-01 → 2026-06-06). Impact-DESC ordering still classifies high-impact reports first. Default cap is 10 canonicals/run (~$0.40/run at gpt-5-mini rates) so wall-clock fits under Vercel Hobby's 60s `maxDuration`; the every-6h cadence (≤40 obs/day) is the throughput lever. Override with `?limit=` up to 100 on Pro plans, or the threshold with `?minImpactScore=` / `CLASSIFY_BACKFILL_MIN_IMPACT`. Run summary is written to `scrape_logs` with `source_id = NULL`; `/api/stats` explicitly excludes those rows from the "Last synced" chip so backfill runs don't masquerade as scrapes. See §3.5 for the full scheduled-jobs contract and `lib/classification/backfill-candidates.ts` for the pure mv-row → `ClassificationCandidate` projection (env/repro derivation goes through the shared helpers in `lib/classification/candidate.ts` so this path and `lib/scrapers/index.ts → buildClassificationCandidate` cannot drift).
...
| `/api/cron/classify-backfill` | `0 3,9,15,21 * * *` (every 6 h, staggered 3 h from scrape) | Walks `mv_observation_current` for `is_canonical = true AND llm_classified_at IS NULL`, classifies up to `DEFAULT_LIMIT` (10) rows in impact-DESC order via the same queue with the dedupe guard, refreshes MVs. **Impact-ungated by default** (`minImpactScore=0`) so the below-threshold long tail is reached — the old `impact_score >= 6` gate let coverage *regress* as the corpus grew (16% → 10.9% between 2026-05-01 and 2026-06-06). Override per-run with `?minImpactScore=` or globally with `CLASSIFY_BACKFILL_MIN_IMPACT`. The shared `MIN_IMPACT_SCORE` policy default (banner, admin) is unchanged. | NULL (distinguishes from scrape rows; `/api/stats` lastScrape filter excludes NULL `source_id`) | `CLASSIFY_BACKFILL_DISABLED=1` (preferred — does not break ingest classifier or SignalLayers Refresh) |
