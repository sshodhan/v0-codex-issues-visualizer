...
6. The ingest queue only enqueues *newly captured* observations. The long tail (rows ingested before the classifier was wired up, or whose first attempt errored) is reached by a separate daily Vercel cron at `/api/cron/classify-backfill` (03:00 UTC, after the last scrape tick of the day). It walks `mv_observation_current` for rows where `llm_classified_at IS NULL AND impact_score >= 6`, sorts by `impact_score DESC, published_at DESC`, and routes the top N through the same `processObservationClassificationQueue()` helper with `reclassifyExisting: false`. Default cap is 25 rows/run (~$1/run at gpt-5-mini rates); override with `?limit=`. Run summary is written to `scrape_logs` with `source_id = NULL` so the dashboard's "Last sync" chip and admin views surface backfill activity alongside scrape activity. See `lib/classification/backfill-candidates.ts` for the pure mv-row → `ClassificationCandidate` projection (env/repro derivation mirrors `lib/scrapers/index.ts → buildClassificationCandidate`).
