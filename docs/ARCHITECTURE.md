...
- `algorithm-versions.ts` — central `CURRENT_VERSIONS` registry (the single source of truth for the derivation version every enrich pass stamps on its writes). Current state: `sentiment: "v2"`, `category: "v2"`, `impact: "v2"`, `competitor_mention: "v2"` (all bumped together in `scripts/011_algorithm_v2_bump.sql` after the eye-test tuning), `classification: "v1"`, `observation_embedding: "v1"` (shipped in `scripts/012_semantic_clustering.sql`), `semantic_cluster_label: "v2"` (bumped 2026-04 alongside the deterministic-fallback labeller — see `docs/CLUSTERING_DESIGN.md` §4.4), `bug_fingerprint: "v1"` (shipped in `scripts/013_bug_fingerprints.sql`). Bumping here is the only way to trigger a new derivation row shape — the registry file is authoritative; 007's seed + delta migrations (011+) must stay consistent with it. The schema-verifier manifest in `lib/schema/expected-manifest.ts` carries the same expected versions; `tests/algorithm-version-manifest-contract.test.ts` fails the build if either half drifts.
