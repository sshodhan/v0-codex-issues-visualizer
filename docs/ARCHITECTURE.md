...
- `derivations.ts` — `recordSentiment(observationId, result)`, `recordCategory(observationId, categoryId, confidence, evidence?)`, `recordImpact`, `recordCompetitorMention`, `recordBugFingerprint(observationId, payload)`, `recordClassification(payload, priorId?)`, `recordClassificationReview(classificationId, review)`. Each pulls the current `algorithm_version` from `algorithm-versions.ts` and stamps the row. `recordCategory`'s `evidence` arg is the JSONB payload returned by `categorizeIssue` (matched phrases + per-slug scores + margin + runner-up); persisted into `category_assignments.evidence` (added in `scripts/026_category_assignments_evidence.sql`) so admin debugging can answer "why did this row classify as X?" with a single SQL query. `recordBugFingerprint` only takes regex-derived signals — the LLM classifier's output is not denormalized onto `bug_fingerprints` (see §3.2).
...
- `category_assignments (observation_id, algorithm_version, category_id, confidence NUMERIC(3,2), evidence JSONB, computed_at)`. `evidence` (added in `scripts/026`) carries the v5+ regex matcher's structured output: `{ matched_phrases[{phrase, weight, in: "title"|"body"}], scores{slug→int}, margin, runner_up, threshold }`. Nullable — pre-v5 rows persist with NULL.
