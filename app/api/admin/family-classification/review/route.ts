...
const REVIEW_DECISION_SET = new Set<string>(REVIEW_DECISIONS as readonly string[])
const ERROR_SOURCE_SET = new Set<string>(ERROR_SOURCES as readonly string[])
...
export async function POST(request: NextRequest) {
  ...
  logServer({
    ...
    data: {
      ...
      cluster_id: result.value.cluster_id,
      review_verdict: result.value.review_verdict,
      review_decision: result.value.review_decision,
      error_source: result.value.error_source,
...
export async function GET(request: NextRequest) {
  ...
  const reviewDecision = url.searchParams.get("reviewDecision")?.trim() || null
  const qualityBucket = url.searchParams.get("qualityBucket")?.trim() || null
  const errorSource = url.searchParams.get("errorSource")?.trim() || null
  ...
  if (reviewDecision && !REVIEW_DECISION_SET.has(reviewDecision)) {
    return NextResponse.json(
      {
        error: `reviewDecision must be one of: ${[...REVIEW_DECISION_SET].join(", ")}`,
      },
      { status: 400 },
    )
  }
  ...
  if (errorSource && !ERROR_SOURCE_SET.has(errorSource)) {
    return NextResponse.json(
      { error: `errorSource must be one of: ${[...ERROR_SOURCE_SET].join(", ")}` },
      { status: 400 },
    )
  }
  ...
  let query = supabase
    ...
    .select(
      "id, classification_id, cluster_id, review_verdict, review_decision, expected_family_kind, actual_family_kind, quality_bucket, error_source, error_reason, notes, reviewed_by, reviewed_at, evidence_snapshot",
    )
  ...
  if (reviewDecision) query = query.eq("review_decision", reviewDecision)
  if (qualityBucket) query = query.eq("quality_bucket", qualityBucket)
  if (errorSource) query = query.eq("error_source", errorSource)
